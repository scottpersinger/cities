...
class Session:
    def __init__(self, key: str, options: ClaudeAgentOptions):
        ...
        self._turn = 0
        # Monotonic timestamp of the last completed turn — used by the
        # SessionManager reaper to evict sessions idle longer than the TTL.
...
class SessionManager:
    def __init__(
        ...
        self._lock = asyncio.Lock()
        # If set, the reaper closes sessions idle longer than this.
    ...
    def start_reaper(self) -> None:
        """Start the background idle-session reaper if `idle_timeout_s` is set.

        Idempotent — safe to call multiple times. Must be called from within
        a running asyncio event loop.
        """
        if self._idle_timeout_s is None or self._idle_timeout_s <= 0:
            return
    ...
    async def _reap_idle(self) -> None:
        ...
        for key, session in to_close:
            ...
            try:
                ...
                log.info("reaped idle session key=%s idle=%.0fs", key, idle)
